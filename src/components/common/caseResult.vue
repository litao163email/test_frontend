<template>
	<el-tabs model-value="rb" style="min-height: 300px;" type="border-card" value="rb" size="mini">
		<el-tab-pane label="响应体" name="rb">
			<div v-if="result.response_header">
				...
				<div v-else><Editor :readOnly="true" v-model="result.response_body" lang="html" theme="chrome" height="500px"></Editor></div>
			</div>
		</el-tab-pane>
			...
			<el-scrollbar height="400px">
				<div class="tab-box-sli">
					<div v-for="(item, index) in result.log_data">
						...
						<el-tag style="margin-top: 3px;" v-else-if="item[0] === 'INFO'" type="primary">{{ item[1] }}</el-tag>
	...
	<div style="margin-top: 10px;width: 100%;text-align: center;" v-if="result.state === '失败' && showbtn">
		<el-button @click="addBugDlg = true" type="primary" plain size="mini">提交bug</el-button>
	</div>
	<!-- 添加bug的弹框 -->
