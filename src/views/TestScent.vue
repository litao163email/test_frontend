	...
	<el-row>
		...
		<el-col :span="4">
			<div class="data_box">
				...
				<el-button @click="addScent" type="primary" plain size="mini" style="width: 100%;" icon="el-icon-plus">添加测试场景</el-button>
				<el-scrollbar height="calc(100vh - 134px);">
					<el-menu :default-active="active" class="el-menu-vertical-demo" active-text-color="#0044aa" text-color="#000000" style="background: none;border: none;">
		...
		<el-col :span="20" style="padding: 0 20px;">
			<div class="scent-data-box" v-if="steps">
				...
				<el-row :gutter="5" style="margin-bottom: 20px;">
					<el-col :span="12">
						<el-input v-model="scentInfo.name" size="mini">
							<template #prepend>
								场景名称
							</template>
							<template #append>
								<el-button @click="saveScent" plain icon="el-icon-circle-check" size="mini" type="primary" >保存</el-button>
							</template>
						</el-input>
					</el-col>
					<el-col :span="12" style="text-align: right;">
						<el-button @click="runScent" plain size="mini" type="primary" icon="el-icon-s-promotion">运行</el-button>
						<el-button @click="deleteScent" plain icon="el-icon-delete" size="mini" type="danger">删除</el-button>
					</el-col>
				</el-row>
					...
					<el-table :data="steps" row-key="id" style="width: 100%;margin-bottom: 10px;" :show-header="false">
						<el-table-column>
							<template #default="scope">
								<span class="el-icon-upload" style="color: #0044aa;font-weight: bold;font-size: 14px;">{{ '步骤' +  (scope.$index+1) + ': ' }}</span>
								<span style="font-weight: bold;font-size: 14px;">{{ scope.row.stepInfo.title }}</span>
							</template>
						</el-table-column>
						<el-table-column width="200px">
							<template #default="scope">
								<el-button @click="clickEditStep(scope.row.stepInfo.id)" plain size="mini" type="primary" icon="el-icon-edit-outline">编辑</el-button>
								<el-button @click="clickDelete(scope.row.id)" plain size="mini" type="danger" icon="el-icon-delete">删除</el-button>
							</template>
						</el-table-column>
					</el-table>
					<el-button @click="addStepDlg = true" plain icon="el-icon-plus" size="mini" type="primary">新增步骤</el-button>
				</el-scrollbar>
			</div>
		</el-col>
	</el-row>
		...
		<template #default>
			<el-tabs type="card" style="height: calc(100vh - 100px);">
				<el-tab-pane label="内部接口">
					<el-tree ref="treeM1" :data="interfaces1" show-checkbox :props="addTreeProps" node-key="id" default-expand-all highlight-current>
						<template #default="{ node, data }">
							<span class="custom-tree-node">
								<div v-if="data.name">
									<b style="color:#0044aa ;">
				...
				<el-tab-pane label="外部接口" name="second">
					<el-tree ref="treeM2" :data="interfaces2" show-checkbox :props="addTreeProps" node-key="id" default-expand-all :expand-on-click-node="false">
						<template #default="{ node, data }">
							<span class="custom-tree-node">
								<div v-if="data.name">
									<b style="color:#0044aa ;">
			...
			<div class="add-btns">
				<el-tooltip class="item" effect="dark" content="将选择的用例,加入到测试场景中" placement="top-start">
					<el-button plain type="primary" size="mini" @click="addToScent('add')">确认添加</el-button>
				</el-tooltip>
